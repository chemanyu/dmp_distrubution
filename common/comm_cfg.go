...
func init() {
	app_path := Get_main_path()
	// 支持go run 和 go test
	work_path := Get_work_path()
	// config_path := "etc/conf.ini"
	config_path := fmt.Sprintf("etc%sconf.ini", string(filepath.Separator))

	// 现在应用对应目录查找
	cfg, err := goconfig.LoadConfigFile(Join_path(app_path, config_path))

	// 查找失败，在当前目录查找
	if err != nil {
		cfg, err = goconfig.LoadConfigFile(Join_path(work_path, config_path))
	}

	// 当前目录查找失败，在去当前目录上一级目录查找
	if err != nil {
		cfg, err = goconfig.LoadConfigFile(Join_path(filepath.Dir(work_path), config_path))
	}

	if err != nil {
		program_name := Get_main_name()
