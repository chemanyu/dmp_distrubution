...
func Get_work_path() string {
	...
	//work_path_dir := filepath.Dir(work_path)
	//return work_path_dir
	return work_path
}

func Join_path(arr_str ...string) string {
	return strings.Join(arr_str, string(filepath.Separator))
}
...
// check ipv4 address
func Is_ipv4(s_ip string) bool {
	segs := strings.Split(s_ip, ".")
	if len(segs) != 4 {
		return false
	}
...
func Rc4(key, src []byte) ([]byte, error) {
...
func Get_m1(imei string) string {
	md5Ctx := md5.New()
	md5Ctx.Write([]byte(imei))
